...
ADMIN_IDS = [1242134309305974886]  # Replace with actual admin IDs
...
async def deploy_with_os(interaction, os_type, ram, cpu, user_id, user, container_name, expiry_date):
    ...
    if ssh_session_line:
        ...
        dm_embed.add_field(name="🔒 Password", value="rexnodes", inline=False)
...
async def sendvps(
    ...
    if interaction.user.id not in ADMIN_IDS:
        embed = discord.Embed(
            title="❌ Access Denied",
            description="Only GamerFonzee admins can use this command.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    embed.set_footer(text="🔐 Safe your details | Powered by REX NODES")
